	...
	SubShader
	{
		...
		Pass
		{
			...
			float3 light(float3 p,float3 view) {
				float texscale = 5.0;
				float3 normal = gn(p);
				float3 tan = normalize(cross(normal,normalize(float3(1,1,0))));
				float3 bin = cross(tan,normal);
				float4x4  mat = float4x4(float4(tan, 0),float4(bin, 0),float4(normal, 0),float4(0, 0, 0, 1));
				mat = transpose(mat);
				float3 normaltex = UnpackNormal(tex2Dlod(_NormalTex, float4(texscale * (p.xz + p.y), 0, 0)));
				normal = normalize(mul(mat,float4(normaltex, 1)).xyz);
				...
				float3 maintex = tex2Dlod(_MainTex, float4(texscale * (p.xz+p.y), 0, 0)).xyz;

Shader "Butadiene/raymarchTexSample"
{
	Properties
	{
	...
	SubShader
	{
		...
		Pass
		{
			...
			float3 light(float3 p,float3 view) {//ライティング
				...
				//接空間の計算
				float3 tan = normalize(cross(normal,normalize(float3(1,1,0))));//適当な方向のベクトルを使って接線を計算
				float3 bin = cross(tan,normal);
				float4x4  mat = float4x4(float4(tan, 0),float4(bin, 0),float4(normal, 0),float4(0, 0, 0, 1));//変換行列の作成
				mat = transpose(mat);

				float3 normaltex = UnpackNormal(tex2Dlod(_NormalTex, float4(texscale * (p.xz + p.y), 0, 0)));//法線マップの取得
				normal = normalize(mul(mat,float4(normaltex, 1)).xyz);//法線マップの結果を使うことで法線を計算しなおす
				...
				float3 maintex = tex2Dlod(_MainTex, float4(texscale * (p.xz+p.y), 0, 0)).xyz;//テクスチャの取得
				float3 col =maintex * (NdotL + spec);
				...
				return col * 0.5 + refcol;//リフレクションの計算結果を足す
			}

//スライダー本体の位置を示すためのShader
Shader "ButadieneSlidersystem/targetpositionparent"
{
	...
	SubShader
	{
		...
		Pass
		{
			...
			{
				v2f o;
				o.uv = v.uv * float2(4/256.,1/32.0) + float2(4*_Index/256.,(31-(_Indey+14))/32.0); // 指定されたIDを受けて、スクリーンスペース上で表示する位置を決めます.
				//(スライダーそのものを表示するShaderよりもy座標で少し下((_Indey+14)の14)に書き込むようにしています。)
				o.vertex = float4(o.uv.x*2-1,1-o.uv.y*2,0,1);
				float orth  = UNITY_MATRIX_P[3][3];//これによって平行投影のカメラにのみ写るようになります。
				if(orth == 0){
